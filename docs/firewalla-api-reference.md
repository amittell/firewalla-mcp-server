...
- A word with no qualifier is sent as it is, and search_rules leaves it to `/v2/rules` and re-checks only the field terms it reads (`action`, `status`, `target.value`), since the fields the API matches free text against are not documented and have not been measured on rules.
...
**Client**: `requestPages` in `src/firewalla/client.ts` asks for at most 500 items per request and follows `next_cursor` up to the limit. It stops at a response with no `next_cursor`, at a page with no items (returning that page's `next_cursor`, so the caller can read on), and at a `next_cursor` it has already sent in the same read: the page at that cursor would come back again, so it returns no cursor. `get_flow_data` and `search_flows` return `coverage` with their flows: `oldest_ts` and `newest_ts`, the oldest and newest `ts` among the flows the API returned (Unix seconds, as sent; ISO strings in `oldest` and `newest`), `api_requests` (requests sent to the API, which count against its 100 per 5 minutes, 429 retries included; a page answered from the client's response cache, `CACHE_TTL` seconds, sends none), `cached_pages` (pages answered from that cache), and `stopped_reason`: `limit_reached`, `no_more_pages`, `repeated_cursor` or `empty_page`. Without a `ts` qualifier the flows cover only the last 24 hours, newest first, so `oldest_ts` shows how far back a read got.

`get_flow_data` streams a limit over 50 (or any limit, with `stream: true`): the page comes back as the first chunk of a session, with `sessionId`, `nextContinuationToken` (the API's `next_cursor`) and `isFinalChunk`, which is true exactly when there is no `nextContinuationToken` (an empty chunk with a cursor is not final, as an empty plain page with a cursor has `has_more: true`), and its flows have the fields of a plain page's (`ts` as an ISO string). `streaming_session_id: <sessionId>` returns the next chunk, of the same size and with the session's query. Sessions are kept by a streaming manager that lasts as long as the API client (one per client, in the server process), but each session has its own lifetime: it expires 10 minutes after its latest chunk, and one that has returned its final chunk (`isFinalChunk: true`) is refused as complete and removed a minute later. Overlapping calls for one session are read one after the other, so they get consecutive chunks. A `cursor` (`pagination.cursor`, or a `nextContinuationToken`) returns the page at that cursor, not streamed, even with a `streaming_session_id`; `stream: false` returns plain pages at any limit, and is refused with a `streaming_session_id` (a session's chunks are streamed).
