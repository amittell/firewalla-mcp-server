  ...
  async execute(args: ToolArgs, firewalla: FirewallaClient): Promise<ToolResponse> {
    try {
      ...
      return this.createSuccessResponse({
        ...
        rules: summaryOnly ? optimizedResponse.results : SafeAccess.safeArrayMap(
          ...
          (rule: any) => ({
            ...
            target: rule.target ? {
              type: SafeAccess.getNestedValue(rule.target, 'type', 'unknown'),
              value: SafeAccess.getNestedValue(rule.target, 'value', 'unknown'),
...
export class GetTargetListsHandler extends BaseToolHandler {
  ...
  async execute(args: ToolArgs, firewalla: FirewallaClient): Promise<ToolResponse> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
      return this.createErrorResponse(`Failed to get target lists: ${errorMessage}`);
    }
  }
}
...
export class GetNetworkRulesSummaryHandler extends BaseToolHandler {
  ...
  async execute(args: ToolArgs, firewalla: FirewallaClient): Promise<ToolResponse> {
    try {
      ...
      const rulesByAction = allRules.reduce((acc: Record<string, number>, rule: any) => {
        const action = SafeAccess.getNestedValue(rule, 'action', 'unknown');
        acc[action] = (acc[action] || 0) + 1;
        return acc;
      }, {});
      
      const rulesByDirection = allRules.reduce((acc: Record<string, number>, rule: any) => {
        const direction = SafeAccess.getNestedValue(rule, 'direction', 'unknown');
        acc[direction] = (acc[direction] || 0) + 1;
        return acc;
      }, {});
      
      const rulesByStatus = allRules.reduce((acc: Record<string, number>, rule: any) => {
        const status = SafeAccess.getNestedValue(rule, 'status', 'active');
        acc[status] = (acc[status] || 0) + 1;
        return acc;
      }, {});
      
      const rulesByTargetType = allRules.reduce((acc: Record<string, number>, rule: any) => {
        const targetType = SafeAccess.getNestedValue(rule, 'target.type', 'unknown');
      ...
      const totalHits = allRules.reduce((sum: number, rule: any) => sum + SafeAccess.getNestedValue(rule, 'hit.count', 0), 0);
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
      return this.createErrorResponse(`Failed to get network rules summary: ${errorMessage}`);
    }
  }
}
...
export class GetMostActiveRulesHandler extends BaseToolHandler {
  ...
  async execute(args: ToolArgs, firewalla: FirewallaClient): Promise<ToolResponse> {
    try {
      ...
      return this.createSuccessResponse({
        total_rules_analyzed: SafeAccess.safeArrayAccess(allRulesResponse.results, (arr) => arr.length, 0),
        ...
        rules: SafeAccess.safeArrayMap(
          activeRules,
          (rule: any) => {
            const targetValue = SafeAccess.getNestedValue(rule, 'target.value', '');
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
      return this.createErrorResponse(`Failed to get most active rules: ${errorMessage}`);
    }
  }
}
    ...
    try {
      ...
      const recentRules = SafeAccess.safeArrayFilter(
        ...
        (rule: any) => {
          ...
          const modified = includeModified && updateTs >= hoursAgoTs && updateTs > ts;
      ...
      return this.createSuccessResponse({
        ...
        summary: {
          newly_created: recentRules.filter((r: any) => {
            ...
            return ts >= hoursAgoTs && (updateTs <= ts || updateTs < hoursAgoTs);
          }).length,
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
      return this.createErrorResponse(`Failed to get recent rules: ${errorMessage}`);
    }
  }
}